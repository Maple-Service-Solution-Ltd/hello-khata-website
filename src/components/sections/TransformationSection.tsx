...
export default function TransformationSection() {
  ...
  const chaosCards = [
    { icon: FileText, label: t('transformation.chaosCards.dues'), rotation: -6, x: 5, y: -10 },
    { icon: Calculator, label: t('transformation.chaosCards.stock'), rotation: 4, x: 60, y: 20 },
    { icon: NotebookPen, label: t('transformation.chaosCards.profit'), rotation: -3, x: 30, y: 50 },
  ]
  ...
  // Single scroll tracker — used for all transforms below
  const { scrollYProgress } = useScroll({
    target: sectionRef,
    offset: ['start end', 'end start'],
  })

  // Before half: fade + scale + slight rotate as it scrolls away
  ...
  // Parallax: before section moves up slightly
  const beforeY = useTransform(scrollYProgress, [0, 0.5], [0, -60])

  // Horizon line vertical drift
  const horizonY = useTransform(scrollYProgress, [0, 0.5, 1], [40, 0, -40])

  // After half: fade in + rise up
  ...
  // Parallax: after section moves down slightly as user scrolls in
  const afterParallaxY = useTransform(scrollYProgress, [0.5, 1], [0, 80])
            ...
            {chaosCards.map((card, i) => (
              ...
              >
                ...
                >
                  ...
                  <span className="font-bengali text-[15px]" style={{ color: 'rgba(250,247,240,0.7)' }}>
                    {card.label}
                  </span>
                </div>
              </motion.div>
          ...
          <div className="flex flex-col gap-5">
            ...
            {afterFeatures.map((feature, i) => (
              ...
              >
                ...
                <span className="font-bengali text-[18px]" style={{ color: 'var(--text-ink)' }}>
                  {feature}
                </span>
              </motion.div>
            ))}
